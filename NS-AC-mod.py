...
class LowerBoundary(fe.SubDomain):
    def inside(self, x, on_boundary):
        return on_boundary and x[1] >= -abs(eps) and x[1] <= abs(eps)
...
lin_form_AC = c_n * q * fe.dx - k*q*fe.dot(vel_n, fe.grad(c_n))*fe.dx\
    - k*fe.dot(fe.grad(q), mobility(c_n)*fe.grad(c_n))*fe.dx\
        - 0.5*k**2 * fe.dot(vel_n, fe.grad(q)) * fe.dot(vel_n, fe.grad(c_n)) *fe.dx\
            - k*np.cos(theta)*q*mobility(c_n)*4*c_n*(1 - c_n)/eps*ds_bottom

lin_form_mu = 4*beta*(c_n - 1)*(c_n - 0)*(c_n - 0.5)*v*fe.dx\
    + kappa*fe.dot(fe.grad(c_n),fe.grad(v))*fe.dx\
